...
# 6. Indicador Ratio UF/EUR (cuántos euros es 1 UF)
val_uf_eur = 0.0
if val_uf_hoy > 0 and val_eur_hoy > 0:
    val_uf_eur = val_uf_hoy / val_eur_hoy

uf_eur_txt = f"{val_uf_eur:,.2f} EUR" if val_uf_eur > 0 else "No disponible"

st.sidebar.markdown(f"""
    <div style="background-color: #e8f0fe; padding: 12px 15px; border-radius: 8px; border: 1px solid #d2e3fc; display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
        <span style="font-size: 28px; line-height: 1;">🪙</span>
        <div>
            <div style="font-size: 13px; color: #174ea6; font-weight: bold;">Ratio UF/EUR:</div>
            <div style="font-size: 16px; color: #202124; font-weight: bold; margin-top: 2px;">{uf_eur_txt}</div>
        </div>
    </div>
""", unsafe_allow_html=True)
...
# 7. Selector de Fuente de Datos y Token
...
if not submitted:
    ...
    st.markdown("""
        <h1 style='font-size: 40px; color: #202124; margin-bottom: 5px;'>
    ...
    st.markdown("### 🪭 Abanico de Escenarios de Monte Carlo")
    if moneda_contrato == "CLP":
    ...
    with st.expander("ℹ️ Nota Metodológica y Definición de Escenarios (Detalle Técnico para TFM)"):
        ...
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #332288; font-size: 13px; color: #333; line-height: 1.5; font-family: sans-serif;">
            ...
            • <b>Escenario optimista (P10 - Cota Inferior):</b> Se deriva del percentil 10 de las trayectorias simuladas mediante Monte Carlo. Respeta estrictamente el principio de <b>nominalidad contractual</b>, fijando el canon en el nivel del tramo anterior cuando las iteraciones presentan reducciones nominales por deflación.<br><br>
